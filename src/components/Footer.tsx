...
export default function Footer({ onOpenCustomFitModal }: FooterProps) {
  ...
  return (
    <footer className="bg-[#F5EFE6] text-[#1E332D] pt-20 pb-10 border-t border-[#C8A366]/30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 pb-16 border-b border-[#C8A366]/30">
          <div>
            ...
            <h3 className="font-serif text-3xl font-normal text-[#1E332D] mb-3">
              Join Archie&apos;s VIP Circle
            </h3>
            <p className="text-xs text-[#4A5D57] font-light leading-relaxed max-w-md mb-6">
            ...
            {!subscribed ? (
              <form onSubmit={handleSubscribe} className="flex gap-2 max-w-md">
                <input
                  ...
                  required
                  className="flex-1 px-4 py-3 bg-[#FAF6F0] border border-[#C8A366]/40 text-xs text-[#555] focus:outline-none focus:ring-1 focus:ring-[#1E332D]"
                />
                <button
                  type="submit"
                  className="px-6 py-3 bg-[#1E332D] text-white font-bold text-xs uppercase tracking-widest hover:bg-[#C8A366] transition-colors flex items-center gap-2"
                >
                  Join <Send className="w-3.5 h-3.5" />
                </button>
              </form>
            ) : (
              <div className="p-4 bg-[#FAF6F0] border border-[#C8A366] text-xs text-[#C8A366] font-semibold max-w-md">
                Welcome to the VIP Circle! You will receive our next collection lookbook directly.
              </div>
            )}
          </div>

          <div className="grid grid-cols-2 gap-8 text-xs text-[#444]">
            <div>
              <h4 className="font-serif text-base font-medium text-black mb-4 tracking-wider uppercase">
                Collections
              </h4>
              ...
              <ul className="space-y-2.5">
                <li>
                  <button
                    ...
                    className="text-[#8C6D27] font-bold hover:underline flex items-center gap-1"
                  >
                    <Sparkles className="w-3 h-3" />
                    Bespoke Measurement Studio
                  </button>
                </li>
                <li>
        ...
        <div className="pt-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <span className="font-serif text-xl tracking-[0.2em] font-light text-black uppercase block">
          ...
          <div className="flex items-center gap-6 text-xs text-[#555]">
            <a
              ...
              className="flex items-center gap-2 hover:text-[#8C6D27] transition-colors font-semibold"
            >
              <InstagramIcon className="w-4 h-4 text-[#8C6D27]" />
              @archie_by_priyanka
            </a>
            <span className="text-[#CCC]">|</span>
            <span className="flex items-center gap-1">
              <Lock className="w-3.5 h-3.5 text-[#8C6D27]" />
              Encrypted Checkout
            </span>
          </div>

          <p className="text-[11px] text-[#777] text-center md:text-right">
